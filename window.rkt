     ...
     (let ([inserted-text (box #f)]
       ...
       (let* ([modifiers (tell #:type _NSUInteger event modifierFlags)]
	      ...
	      [shift?   (bit? modifiers NSShiftKeyMask)]
              ...
              [codes (cond
                      [wheel wheel]
                      [mod-change? (set! kc (tell #:type _ushort event keyCode))
                                   (case kc 
                                     [(56) '(shift)]
                                     [(59) '(control)]
                                     [(60) '(rshift)]
                                     ...
                                     [else '()])]
                      [had-saved-text? str]
                      [(map-key-code (begin (set! kc (tell #:type _ushort event keyCode)) kc))
                       => list]
                      [(string=? "" str) '(#\nul)]
                            ...
                            (let ([c (string-ref str 0)])
                              (or (and control?
                                       ...
                                       (let ([alt-str (tell #:type _NSString event charactersIgnoringModifiers)])
                                         (set! kc (tell #:type _ushort event keyCode))
                                         (and (string? alt-str)
                      ...
                      [else str])])
         (when key-down?
           (newline) (write (list 'codes codes)) (newline)
	   (when kc
             (write (list 'keyCode kc)) (newline)
             (write (list 'mapped-keyCode (map-key-code kc))) (newline)))
         (for/fold ([result dead-key?]) ([one-code codes])
           (or
            ...
            (let-values ([(x y) (send wx window-point-to-view pos)])
			  ...
			  (write (list 'old-other-altgr alt-code)) (newline)
		      ...
		      (define (old-dks-copy) (copy-dead-key-state prev-dks))
                      (define mask ; keep only modifier keys
			(+ modifier-shift-key modifier-option-key modifier-alpha-lock
			   modifier-cmd-key modifier-control-key))
      		      (define kc        (tell #:type _ushort event keyCode))
		      (define mod-ks    (bitwise-and (<< modifiers -8) mask))
		      (define s         (key-translate kc #:modifier-key-state mod-ks 
						       #:dead-key-state dead-key-state))
		      (write (list 's s)) (newline)
			...
			(define s (key-translate kc #:modifier-key-state mod 
						 #:dead-key-state (old-dks-copy)))
			; (write (list 'alternative who s)) (newline)
			(when (> (string-length s) 0)
			  (let ([c (string-ref s 0)])
			    (write (list 'alternative who c)) (newline)
			    (setter c))))
		      ...
		      ; (alternative (lambda (c) (send k set-other-shift-altgr-code c)) shift-mod)
		      ; --- Is computing the main char a good or bad idea ? ---
		      ; For now store it in other-caps-key-code which seems unused(?)
		      (define-values (main-char main-mods) (char->main-char-key+modifiers c))
		      (send k set-other-caps-key-code main-char) ; in lieu of a better place
		      (write (list 'c c 'main main-char)) (newline)))
